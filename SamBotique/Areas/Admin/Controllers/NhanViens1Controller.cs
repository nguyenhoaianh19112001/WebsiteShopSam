...
namespace SamBotique.Areas.Admin.Controllers
{
    public class NhanViens1Controller : Controller
    {
        ...
        {
            var links = from nv in db.NhanViens // lấy toàn bộ liên kết
                        select nv;

            if (!String.IsNullOrEmpty(searchString)) // kiểm tra chuỗi tìm kiếm có rỗng/null hay không
            {
                links = links.Where(s => s.TenNV.Contains(searchString) || s.DiaChi.ToString() == searchString); //lọc theo chuỗi tìm kiếm
            }
        ...
        public ActionResult Create([Bind(Include = "MaNV,TenNV,GioiTinh,DiaChi,SDT,TaiKhoan,MK")] NhanVien nhanVien)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "MaNV,TenNV,GioiTinh,DiaChi,SDT,TaiKhoan,MK")] NhanVien nhanVien)
        {
            if (ModelState.IsValid)
            {
